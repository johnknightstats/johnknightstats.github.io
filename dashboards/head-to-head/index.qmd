...
server <- function(input, output, session) {
  ...
  output$h2h_plot <- renderPlotly({
    ...
    # --- Single-line plot: one marker per year (winner colour) ---

# Apply clash rule for colours (same as you already do)
# Team A always uses home; Team B switches to change if home clashes
col_a <- col_a_home
col_b <- if (tolower(col_b_home) == tolower(col_a_home)) col_b_change else col_b_home
...
# Build a full year range and keep it blank unless BOTH teams are in the league
df_plot <- df %>%
  mutate(
    both_in = a_in & b_in,
    # only plot when both in (shared seasons)
    x_plot = if_else(both_in, year, NA_integer_),
    y_plot = 0,
    winner_class = case_when(
      both_in & finished_highest == a_name ~ "A",
      both_in & finished_highest == b_name ~ "B",
      both_in ~ "T",   # tie (same tier/position or Tier 3 N vs S)
      TRUE ~ NA_character_
    ),
    hover = paste0(
      "<b>Year: </b>", year, "<br>",
      if_else(both_in,
        paste0(
          "<br><b>", a_name, "</b><br>",
          a_season, "<br>",
          "Pos: ", a_pos, " · Pts: ", a_pts, "<br>",
          "W-D-L: ", a_wdl, " · GD: ", a_gd, "<br>",
          "<br><b>", b_name, "</b><br>",
          b_season, "<br>",
          "Pos: ", b_pos, " · Pts: ", b_pts, "<br>",
          "W-D-L: ", b_wdl, " · GD: ", b_gd, "<br>",
          "<br><b>Finished higher:</b> ", finished_highest,
          "<br><b>Reason:</b> ", reason
        ),
        "Not a shared Football League season"
      )
    )
  )

  # Split into traces so the legend is meaningful
  df_a <- df_plot %>% filter(winner_class == "A")
  df_b <- df_plot %>% filter(winner_class == "B")
  df_t <- df_plot %>% filter(winner_class == "T")
  ...
  p <- plot_ly() %>%
    ...
    add_trace(
      ...
      name = "Tie",
      text = ~hover, hoverinfo = "text",
      marker = list(size = 26, color = "gray70", line = list(width = 1, color = "gray40")),
      customdata = ~year
    ) %>%
    layout(
      xaxis = list(title = "", tickmode = "linear", dtick = 5, zeroline = FALSE),
      yaxis = list(title = "", visible = FALSE, range = c(-1, 1)),
      legend = list(orientation = "h", x = 0, y = -0.2),
      margin = list(l = 20, r = 20, t = 10, b = 70)
    )
